#!/bin/sh

# Seed bands that serve as a starting point for the crawl.
MARIANAS_REST=3540418193
KAUNIS_KUOLEMATON=3540373021
DDJ=3540297625
GATECREEPER=3540382449
KANONENFIEBER=3540483079
WAYFARER=3540349766
WHITE_WARD=3540354631
MOKOMA=10087
STAM1NA=33052

PYTHONPATH=src .venv/bin/python -m bandmap crawl --seed $MARIANAS_REST $KAUNIS_KUOLEMATON $DDJ $GATECREEPER $KANONENFIEBER $WAYFARER $WHITE_WARD $MOKOMA $STAM1NA --depth 3 --max-bands 10000
